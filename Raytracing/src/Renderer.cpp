...
int main()
{
	...
	for (int y = Height - 1; y >= 0; --y)
	{
		...
		{			
			...
			//Vector3 color(0,0,0);
			Vector3 color( x/(double)Width , y/(double)Height, .6);
...
HitableList * GetRandomWorld()
{
	Hitable** ModelArrays= new Hitable*[4];
	...
	ModelArrays[i++] = new Sphere(Vector3(0,1,0), 1, new Metal(Vector3(0.8, 0.8, 0.8), 0.0));
